...
formProgressRouter.post('/save-step', isAuthenticated, async (req, res) => {
    ...
    try {
        await db.promise().query(`INSERT INTO user_form_progress (user_id, step_number, form_data)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE step_number = VALUES(step_number), form_data = VALUES(form_data)`, [userId, step_number, JSON.stringify(form_data)]);
...
formProgressRouter.get('/load-form', isAuthenticated, async (req, res) => {
    const userId = req.userId;
    try {
        // Fetch user_form_progress
        const [progressRows] = await db.promise().query('SELECT step_number, form_data FROM user_form_progress WHERE user_id = ?', [userId]);
        let step_number = 0;
        let form_data = {};
...
formProgressRouter.post('/reset-form', isAuthenticated, async (req, res) => {
    const userId = req.userId;
    try {
...
export const handleSiteStatus = async (req, res) => {
...
try {
    ...
    console.error('[DB] Invalid MYSQL_URL format, falling back to localhost. Error:', e.message);
...
export const handleSaveCompanyDetails = async (req, res) => {
    const data = req.body;
    ...
    try {
        ...
        if (logoPathAbs && !logoPathAbs.startsWith('http')) {
            ...
            if (base) {
                if (!logoPathAbs.startsWith('/'))
                    logoPathAbs = '/' + logoPathAbs;
...
export const handleGenerateSite = async (req, res) => {
    try {
        const data = req.body;
        ...
        function absPath(p) {
            ...
            const base = (process.env.BASE_URL || '').replace(/\/$/, '');
            if (!base)
                return p; // No BASE_URL provided: keep relative
            if (!p.startsWith('/'))
                p = '/' + p;
            return base + p;
        }
...
export const handleTimesEdited = async (req, res) => {
    try {
        ...
        const timesEdited = rows[0].times_edited || 0;
        if (timesEdited <= 0) {
...
async function testUserCredentials(siteUrl, username, password) {
    try {
        ...
        if (testResp.ok) {
            ...
            const errorMessage = typeof testData === 'object' && testData !== null && 'message' in testData ? testData.message : JSON.stringify(testData);
